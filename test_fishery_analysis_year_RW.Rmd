...
dat<-dat_tst_fsh%>%
  ...
  mutate(year = year - min(year)  +1,
         day = day - min(day) + 1,
         period = (year-1) * max(day) + day
         )

jagsdat<-list(
  S = max(dat$day) - min(dat$day) + 1,
...
if(!file.exists("fit_RW.RDS")){
  ...
  stop<-Sys.time()
  print(stop-start)
  write.csv(fit$BUGSoutput$summary,"summary_RW.csv")
  saveRDS(fit,"fit_RW.RDS")
}else{
  fit<-readRDS("fit_RW.RDS")
}
...
abundance<-tibble(melt(exp(fit$BUGSoutput$sims.list$b3 - apply(fit$BUGSoutput$sims.list$b3,2,mean) + unique(dat$lm_trs))))%>%
  ...
  bind_cols(dat_trs%>%dplyr::select(year_real=year,trs)%>%group_by(year_real)%>%summarise(trs=first(trs)))%>%
  dplyr::select(!year)%>%
  mutate(year = year_real)
