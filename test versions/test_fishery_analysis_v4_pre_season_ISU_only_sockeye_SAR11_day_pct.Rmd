...
ggplot(abundance,aes(x=year,y=trs))+
  ...
  geom_point()+
  geom_point(mapping=aes(x=max(abundance$year),y=median(forecast$Estimate)),color="red",fill="red")+
  ylim(0,NA)
...
ggplot(day_effect,aes(x=day,y=`0.5`,group=year))+
  ylab("Day of Year Effect (% of test fishery season total passing fishery in that day)")+
  xlab("Day of Year")+
  facet_wrap(~year)+
