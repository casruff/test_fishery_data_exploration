...
forecast <- read_csv(file.path(datadir,"sockeyeforecast.csv"))%>% 
  mutate(log_F=log(Estimate),
         log_F_anom = log_F - dat_trs%>%filter(!is.na(trs))%>%mutate(ml_trs=mean(log(trs)))%>%dplyr::select(ml_trs)%>%unlist()
  )
...
standat<-list(
  ...
  log_trs_anom = dat_trs%>%filter(!is.na(trs))%>%mutate(log_trs_anom =log(trs) - mean(log(trs)))%>%dplyr::select(log_trs_anom)%>%unlist(),
  mu_log_forcast_anom = mean(forecast$log_F_anom),
  sd_log_forcast_anom = sd(forecast$log_F_anom)
)


if(!file.exists("results/fit_pre_season_sockeye_stan.RDS")){
  start<-Sys.time()
  print(start)
  model<-stan_model("stan/model_pre_season_sockeye.stan")
  fit<-sampling(
    object=model,
    data=standat,
    cores = 4,
  ...
  stop<-Sys.time()
  print(stop-start)
  write.csv(summary(fit)$summary,"results/summary_pre_season_sockeye_stan.csv")
  saveRDS(fit,"results/fit_pre_season_sockeye_stan.RDS")
}else{
  fit<-readRDS("results/fit_pre_season_sockeye_stan.RDS")
}

ml_trs=dat_trs%>%filter(!is.na(trs))%>%summarise(ml_trs = mean(log(trs)))%>%dplyr::select(ml_trs)%>%unlist()
...
day_effect<-tibble(melt(exp(extract(fit)$resid_day)))%>%
  rename(draw=iterations,day=Var2,day_effect=value)%>%
  group_by(day)%>%
