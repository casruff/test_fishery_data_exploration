...
# forecast<-read_csv("https://raw.githubusercontent.com/tbuehrens/Skagit-River-Steelhead-Forecast/master/analysis/cache/ensemble_forecast_posterior.csv")%>% mutate(log_F=log(ensemble_forecast_posterior),
#          log_F_anom = log_F - unique(dat$lm_trs)
#          )


write.csv(dat,"sockeye_test_data.csv")
...
jagsdat<-list(
  ...
  lm_trs = dat$lm_trs,
  zl_flow = dat%>%dplyr::select(zl_flow)%>%unlist,
  ##====================================================
  # run with these turned on to show in-season forecast
  #=====================================================
  obs = dat%>%nrow(),
  SockeyeC = dat%>%dplyr::select(SockeyeC)%>%unlist(),
...
write(model,"jags/model_pre_season_sockeye.jags")  
...
if(!file.exists("results/fit_pre_season_sockeye.RDS")){
  start<-Sys.time()
  print(start)
  fit<-jags.parallel(
    data=jagsdat,
    ...
    model.file = "jags/model_pre_season_sockeye.jags",
    n.chains = 4,
    n.iter = 30000,
    n.burnin = 10000,
    n.thin = 1
  ...
  stop<-Sys.time()
  print(stop-start)
  write.csv(fit$BUGSoutput$summary,"results/summary_pre_season_sockeye.csv")
  saveRDS(fit,"results/fit_pre_season_sockeye.RDS")
}else{
  fit<-readRDS("results/fit_pre_season_sockeye.RDS")
}
