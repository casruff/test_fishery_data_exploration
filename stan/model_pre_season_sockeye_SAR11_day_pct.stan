...
transformed parameters{
  ...
  for(i in 2:(S+1)){
    resid_period[i] = resid_period[i-1] + eps_period[i-1] * sigma_init;
  }
  for(i in (S+2):(S*Y)){
    resid_period[i] = phi1 * resid_period[i-1] + phi2 * (resid_period[i-S] - resid_period[i-S-1]) + eps_period[i-1] * sigma_period;
  }
  for(i in 1:Y_obs){
    day_pct[i][1:S] = softmax(resid_period[((i-1)*S+1):(i*S)]);
  }
