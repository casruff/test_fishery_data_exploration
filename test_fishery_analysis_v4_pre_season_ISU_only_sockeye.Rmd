...
jagsdat<-list(
  ...
  #=====================================================
  obs = dat%>%nrow(),
  Sockeye = dat%>%dplyr::select(SockeyeC)%>%unlist(),
...
model<-"model
{
  ...
  ## Removing site effect because only 1 site is fished
  ##random effects{
  #for(i in 2:sites){
  ...
  for(i in 1:obs){
    ...
    log_lambda[i] = intercept + b2[set_num[i]] + log(effort[i]) + b3[year[i]] + resid[day[i]]
