...
model<-"model
{
  ...
  for(i in 2:(S*Y)){
    eps_P[i-1] ~ dnorm(0,1^-1)
    resid_P[i] = phi * resid_P[i-1] + eps_P[i-1] * sigma_P
  }
  ...
  #Likelihood
  for(i in 1:obs){
    log_lambda[i] = intercept + b1[site[i]] + b2[set_num[i]] + log(effort[i]) + b3[year[i]] + resid[day[i]] + resid_P[period[i]]
  ...
  phi ~ dnorm(0,1^-1) T(-1,1)
  r = sigma_disp^-2 #negative binomial overdispersion parameter
  intercept ~ dnorm(0,10^-2)
  b1[1] = 0;
  b2[1] = 0;
  resid[1] = 0;
  resid_P[1] ~ dnorm(0,sigma_P1^-2)
  sigma_P1 = sqrt(sigma_P^2/(1-phi^2))
  for(i in 2:sites){
...
write(model,"jags/model_pre_season2.jags")  
...
if(!file.exists("results/fit_pre_season2.RDS")){
  start<-Sys.time()
  print(start)
  fit<-jags.parallel(
    data=jagsdat,
    ...
    model.file = "jags/model_pre_season2.jags",
    n.chains = 4,
    n.iter = 200,
    n.burnin = 100,
    n.thin = 1
  ...
  stop<-Sys.time()
  print(stop-start)
  write.csv(fit$BUGSoutput$summary,"results/summary_pre_season2.csv")
  saveRDS(fit,"results/fit_pre_season2.RDS")
}else{
  fit<-readRDS("results/fit_pre_season2.RDS")
}
