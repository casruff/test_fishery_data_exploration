...
model<-"model
{
  ...
  for(i in 2:S){
    eps[i-1] ~ dnorm(0,1^-1)
    resid[i] = resid[i-1] + eps[i-1] * sigma^-2
  }
  
  #Likelihood
  for(i in 1:obs){
    log_lambda[i] = mu + b1[site[i]] + b2[set_num[i]] + log(effort[i]) + b3[year[i]] - mean(b3[ylist[1:Y_obs]]) + resid[day[i]] - mean(resid[1:S])
...
if(!file.exists("fit_v2.RDS")){
  ...
  stop<-Sys.time()
  print(stop-start)
  write.csv(fit$BUGSoutput$summary,"summary_v2.csv")
  saveRDS(fit,"fit_v2.RDS")
}else{
