...
preds_yr<-predict.gam(m3b, newdata=newdat, type = "iterms", exclude = c("s(day)","s(site)","s(zl_flow)","s(set_num)"), newdata.guaranteed=TRUE)

preds<-exp(preds_yr[,1]) *  mean(dat_trs$trs) #multiply by mean or geomean?
...
ggplot(data=newdat,aes(x=year, y=trs))+
  geom_point(size=2)+
  geom_point(mapping=aes(x=year,y=preds),size=1.3, color ="blue")+
  geom_line(mapping=aes(x=year,y=preds))
...
preds_yr<-predict.gam(m1b, newdata=newdat, type = "iterms", exclude = c("s(day)","s(site)"), newdata.guaranteed=TRUE)
