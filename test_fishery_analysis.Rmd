...
model<-"model
{
  ...
  #Likelihood
  for(i in 1:obs){
    log_lambda[i] = mu + b1[site[i]] + b2[set_num[i]] + log(effort[i]) + b3[year[i]]-mean(b3[ylist[1:Y_obs]]) + resid[day[i]] 
...
if(!file.exists("fit.RDS")){
  ...
  stop<-Sys.time()
  print(stop-start)
  write.csv(fit$BUGSoutput$summary,"summary.csv")
  saveRDS(fit,"fit.RDS")
}else{
  fit<-readRDS("fit.RDS")
}
